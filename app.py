...
from predefined_clauses import DEFINED_CLAUSES
import json
from utils import is_clause
...
# Set page config
st.set_page_config(page_title="Document Analysis AI", page_icon="📄", layout="wide")
...
def process_documents(uploaded_files: list[UploadedFile]):
    ...
    try:
        with st.status("Processing documents...", expanded=True) as status:
            ...
            for uploaded_file in uploaded_files:
                ...
                for doc in documents:
                    ...
                    if not doc.page_content.strip():
                        status.write(
                            f"⚠️ No text found in {uploaded_file.name}. Skipping..."
                        )
                        continue
            ...
            status.update(
                label="✅ All documents processed successfully!", state="complete"
            )
            return vector_store
        ...
        if question:
            with st.spinner("🔍 Finding answer..."):
                try:
                    ...
                    if is_clause(question):
                        # Reformat if result is clauses
                        llm = ChatOpenAI(temperature=0.7, model="gpt-4o-mini")
                        messages = [
                            {
                                "role": "system",
                                "content": f""" You are a legal assistant. Your task is to understand the given clauses and 
                                Select the predefined clauses that are most relevant to the given data.
                                You are given a set of predefined clauses and data as follows:
                                Predefined Clauses:
                                {DEFINED_CLAUSES}
                        ...
                        clauses = json.loads(llm.invoke(messages).content)
                    ...
                    if "source_documents" in result:
                        ...
                        for doc in result["source_documents"]:
                            # print(doc.metadata)
                            with st.expander(
                                f"Source from {doc.metadata.get('section', 'Document')}"
                            ):
                                ...
                                st.markdown(
                                    f"*Document: {doc.metadata.get('title', 'N/A')}*"
                                )
                                st.markdown(
                                    f"*IsProhibition: {doc.metadata.get('has_prohibitions', 'N/A')}*"
                                )
